...
方法:固定 20 TPS,按梯度递增合成负载(盔甲架/僵尸/掉落物),
每级「预热(丢弃) + 测量(采样 spark/tick query 的 MSPT P95)」,
直到 P95 MSPT 触及阈值(默认 50ms),线性插值得出容量拐点。
...
def sample_mspt(rc, state):
    ...
    tgt = re.search(r"[Tt]arget tick rate:\s*([\d.]+)", out)
    if not p95:
        raise RuntimeError(f"无法解析 tick query 输出: {out[:200]!r}")
    return dict(
        tps=float(tgt.group(1)) if tgt else None,
        p50=float(p50.group(1)) if p50 else (float(avg.group(1)) if avg else None),
...
def main():
    ...
    ap.add_argument("--step", type=int, default=500, help="每级新增实体数(默认 500)")
    ap.add_argument("--max-levels", type=int, default=40)
    ap.add_argument("--warmup", type=int, default=180, help="每级预热秒数,数据丢弃(默认 180)")
    ap.add_argument("--measure", type=int, default=300, help="每级测量秒数(默认 300)")
    ap.add_argument("--interval", type=int, default=10, help="采样间隔秒(默认 10)")
    ...
    print(f"== 容量压测: {args.preset}({p['desc']}), 步长 {args.step}, "
    ...
    try:
        for level in range(1, args.max_levels + 1):
            target = level * args.step
            print(f"\n[级 {level}] 目标 {target} 个 {args.preset}")
            spawn(rc, args.preset, args.step)
            ...
            print(f"  预热 {args.warmup}s ...")
            time.sleep(args.warmup)
            print(f"  测量 {args.measure}s ...")
    ...
    with open(summary_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["level", "count", "p50_ms", "p95_ms", "p95_max_ms", "tps", "steal_pct"])
        for r in results:
            w.writerow([r["level"], r["count"], r["p50"], r["p95"], r["p95_max"], r["tps"], r["steal"]])

    print(f"\n== 结束({stopped}),共 {len(results)} 级 ==")
    print(f"{'count':>8} {'P50':>8} {'P95':>8} {'TPS':>6}")
    for r in results:
        print(f"{r['count']:>8} {r['p50']!s:>8} {r['p95']!s:>8} {r['tps']!s:>6}")

    over = [r for r in results if r["p95"] >= args.threshold]
    under = [r for r in results if r["p95"] < args.threshold]
    if over and under:
        lo, hi = under[-1], over[0]
        cap = lo["count"] + (args.threshold - lo["p95"]) * (hi["count"] - lo["count"]) / (hi["p95"] - lo["p95"])
        print(f"\n容量拐点(P95={args.threshold}ms 插值): 约 {cap:.0f} 个 {args.preset}")
    elif under:
        print(f"\n未触及阈值,容量 > {under[-1]['count']} 个 {args.preset}(加大 --step 或 --max-levels)")
    else:
        print("\n第一级就超阈值,减小 --step 重测")
    print(f"\n明细: {samples_path}\n汇总: {summary_path}")
